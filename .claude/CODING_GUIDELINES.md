...
2. Add the handler in BlauTrigger's `OnDataRecv()` switch/case.
...
```cpp
Serial.printf("[BlauTrigger] OnDataRecv: from %02X:%02X, seq=%d, type=0x%02X\n",
...
- **No `delay()`** in `loop()` on BlauTrigger.
